...
public List<UserBean> searhUser(UserBean userBean) {
	Session session = this.sessionFactory.getCurrentSession();
	
	StringBuilder querryBuilder=new StringBuilder();
	
	querryBuilder.append("SELECT * FROM user_table where otp=:otp and adhar_number=:adharNumber and profile='user'");
	
	SQLQuery query
	 = session.createSQLQuery(querryBuilder.toString()).addEntity(UserBean.class);
	
	query.setParameter("adharNumber",userBean.getAdharNumber());
	query.setParameter("otp",userBean.getOtp());
...
public List<UserBean> adharSearch(UserBean userBean) {
	Session session = this.sessionFactory.getCurrentSession();
	
	StringBuilder querryBuilder=new StringBuilder();
	
	querryBuilder.append("SELECT * FROM user_table where  adhar_number=:adharNumber and profile='user' and verify_user=''YES' ");
