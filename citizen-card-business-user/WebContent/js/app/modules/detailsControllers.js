app.controller("detailsControllers", [
		...
		function($scope, $location, mainService, $log, localStorageService, $timeout, toaster, $rootScope)
		{
			...
			$scope.adharSearch = function(data)
			{
				$scope.RequestWrapper = {
						userId : ''
					}
				$scope.RequestWrapper.userId=document.getElementById("adhr").value;
				
			
				console.info("In detailsControllers ",$scope.RequestWrapper);
				mainService.adharSearch($scope.RequestWrapper)
				.then(
						function(result) {
						$scope.resultData=result;
							$log.info("---Inside $scope.status---"+$scope.resultData);
							...
							if(resultData="success")
							{
							alert("OTP Send To Register Email Successfully")
							}
						else
							{
							alert("Failure.. Kindly Check With DB ")
			...
			$scope.verifyOTP = function(data)
			{
				...
				$scope.RequestWrapper.userId=document.getElementById("adhr").value;
				$scope.RequestWrapper.type=document.getElementById("otp").value;
			
				console.info("In detailsControllers ",$scope.RequestWrapper);
				mainService.verifyOTP($scope.RequestWrapper)
				.then(
						function(result) {
